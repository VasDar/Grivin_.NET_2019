namespace Grivin_Vasiliy_Task14_15
{
	partial class DataOrRam
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.btnRAM = new System.Windows.Forms.Button();
			this.btmDB = new System.Windows.Forms.Button();
			this.label1 = new System.Windows.Forms.Label();
			this.SuspendLayout();
			// 
			// btnRAM
			// 
			this.btnRAM.Location = new System.Drawing.Point(25, 111);
			this.btnRAM.Name = "btnRAM";
			this.btnRAM.Size = new System.Drawing.Size(75, 23);
			this.btnRAM.TabIndex = 0;
			this.btnRAM.Text = "RAM";
			this.btnRAM.UseVisualStyleBackColor = true;
			// 
			// btmDB
			// 
			this.btmDB.Location = new System.Drawing.Point(197, 111);
			this.btmDB.Name = "btmDB";
			this.btmDB.Size = new System.Drawing.Size(75, 23);
			this.btmDB.TabIndex = 1;
			this.btmDB.Text = "Data Base";
			this.btmDB.UseVisualStyleBackColor = true;
			this.btmDB.Click += new System.EventHandler(this.btmDB_Click);
			// 
			// label1
			// 
			this.label1.AutoSize = true;
			this.label1.Location = new System.Drawing.Point(47, 50);
			this.label1.Name = "label1";
			this.label1.Size = new System.Drawing.Size(195, 13);
			this.label1.TabIndex = 2;
			this.label1.Text = "Какую память хотите использовать?";
			// 
			// DataOrRam
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(295, 172);
			this.Controls.Add(this.label1);
			this.Controls.Add(this.btmDB);
			this.Controls.Add(this.btnRAM);
			this.Name = "DataOrRam";
			this.Text = "DataOrRam";
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		private System.Windows.Forms.Button btnRAM;
		private System.Windows.Forms.Button btmDB;
		private System.Windows.Forms.Label label1;
	}
}